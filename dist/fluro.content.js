...
angular.module('fluro.content').service('FluroContentRetrieval', function($cacheFactory, Fluro, $q, $http) {
    ...
    controller.query = function(queryDetails, typeName, id) {
        ...
        if (id) {
            $http.get(Fluro.apiURL + '/content/_query/' + id).then(function(res) {
                deferred.resolve(res.data);
            });
        }

        if (typeName && typeName.length) {
            $http.post(Fluro.apiURL + '/content/' + typeName + '/_query', queryDetails).then(function(res) {
                deferred.resolve(res.data);
            });
        } else {
            $http.post(Fluro.apiURL + '/content/_query', queryDetails).then(function(res) {
                deferred.resolve(res.data);
            });
        }
